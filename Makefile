...
KIND_CLUSTER ?= flux-externalsource-controller-e2e
...
.PHONY: vulncheck
vulncheck: govulncheck ## Check for vulnerabilities in dependencies
	$(GOVULNCHECK) ./...
...
docker-build: ## Build docker image with the manager.
	DOCKER_BUILDKIT=1 $(CONTAINER_TOOL) build --platform=linux/amd64 -t ${IMG} .
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name flux-externalsource-controller-builder
	$(CONTAINER_TOOL) buildx use flux-externalsource-controller-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm flux-externalsource-controller-builder
	rm Dockerfile.cross
...
uninstall: manifests kustomize ## Uninstall CRDs from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	@out="$$( $(KUSTOMIZE) build config/crd 2>/dev/null || true )"; \
	if [ -n "$$out" ]; then echo "$$out" | $(KUBECTL) delete --ignore-not-found=$(ignore-not-found) -f - || true; else echo "No CRDs to delete; skipping."; fi
